...
3. **Clearance listing accuracy** — *(2026-08-08: listing now hidden pending a recount; see ACTION_PLAN A-16. The 179 photos are not in this repo — git records only 6 — so the recount is blocked until they are supplied. Of those 6, labels use two schemes: `i6.5` is a screen-size code, `IP11 pro` a model, which is probably why the listing names models it may never have had.)* — a survey of 45% of the stock photos found the listing describes a phone-case lot, but the stock also contains car mounts and USB cables; the models named (Samsung S9, iPhone XS Max) were not seen while OnePlus models present are unmentioned; and `stock = 179` appears to be a *photo* count — front/back pairing suggests roughly **74** actual items. Unresolved and an overselling risk.
